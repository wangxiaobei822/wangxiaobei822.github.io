...
- [Future Impact Decomposition in Request-level Recommendations](https://arxiv.org/abs/2401.16108) (Work was done at Kuaishou Technology)<br>
  ![](../images/item.png)
  We specify the challenge of inconsistency between users' item-wise view and the recommender system's list-wise view, and formulate a more practical request-level MDP formulation with item-wise reward. To solve the advantage mentioned above, we propose an item-decomposed advantage actor-critic framework and verify its superiority on multiple public datasets and an online A/B test.<br>
...
We prompt LLM (fully-parameterized version) to generate product titles based on product information. Additionally, we conduct research on the current status and key technologies of Chinese open-source large language models, such as ChatGLM. The content generated by LLM serves as a dataset for fine-tuning a smaller version of the LLM  to create a compact, specialized model for a specific vertical domain.
...
![](../images/AutoScale.png)
 We propose a novel framework, AutoScale, providing a unified search space of 3D Parallel and getting rid of the dependence on the mathematical models of current auto parallel algorithms at the same time. AutoScale consists of three extendable modules, namely Search Space, Cost Model, and Search Module, which regards the 3D Parallel from a unified formulation and utilizes paralleled search algorithms to scale search modules automatically.  
